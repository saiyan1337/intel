...
def print_callback(message, context):
    ...
    if message['message_type'] == "certificate_update":
        ...
        #sys.stdout.write(u"[{}] {} (SAN: {})\n".format(datetime.datetime.now().strftime('%m/%d/%y %H:%M:%S'), f" SITE {domain}", ", ".join(message['data']['leaf_cert']['all_domains'][1:])))
        found = re.findall(dois,domain)
        
        if len(found) > 0:
            print(f"---------------\n-----------------\n---------------\n")
            with open (f"{filename}.txt","a") as logfile:
                logfile.write(f"{found} : {domain}\n")
            sys.stdout.write(u"[{}] {} (SAN: {})\n".format(datetime.datetime.now().strftime('%m/%d/%y %H:%M:%S'), f"trigger : {found} SITE : {domain}", ", ".join(message['data']['leaf_cert']['all_domains'][1:])))
